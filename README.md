...
浏览器版是免费的本地处理演示；当前建议用创作者版 ¥29/月、小团队版 ¥99/月做第一轮支付意愿实验，价格仍标记为实验假设。桌面端已包含套餐目录、有效期和 HMAC 签名授权核心；已核验的手工试单可由私有环境运行 `scripts/issue_license.py` 签发短期令牌，桌面工作台粘贴后按签名套餐更新批次上限。没有授权令牌时始终回落到受限试用。支付渠道、订单回调、退款和商家主体尚未接入，不能把 GitHub Pages 的前端状态当作商业授权。
