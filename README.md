...
桌面端由 `build_nuitka.py` 按系统生成对应应用格式（Windows `rembg-ui.exe` / Linux `rembg-ui.bin` / macOS `rembg-ui.app`，CI 压缩为 `.app.zip`）。Android 使用独立原生端侧实现，按 ABI 拆分 APK 并同时生成 AAB；原生端支持单图、最多 10 张顺序批处理、重启恢复、系统分享和 ZIP 清单导出。浏览器版构建为可安装的 WASM PWA。
