...
尚未移植：可迁移的项目备份、桌面端模型选择。浏览器版已提供会话内擦除/恢复、撤销/重做、图像修订协议；GitHub Pages 版本用于公开演示，付费 SaaS 托管需另行安排。
...
验证：32 项 Node 测试通过；真实浏览器执行擦除、撤销、重做、保存、刷新、确认失效、恢复原图笔画及重新确认导出。导出 ZIP CRC 校验通过，交付清单 imageRevision=reviewedRevision=2。修订后的模块使用统一 URL 版本，避免已打开页面混用旧依赖。
