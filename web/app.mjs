...
import {openProjectStore,PROJECT_SCHEMA_VERSION} from './project-store.mjs';
...
function updateControls(){
 const done=batch.filter(item=>item.status==='done'),reviewed=done.filter(item=>item.reviewed);
 ...
 for(const check of results.querySelectorAll('input[type=checkbox]'))check.disabled=locked||conflict;
...
function imagePreview(blob,label,filename){
 ...
 caption.textContent=label;image.alt=`${filename} · ${label}`;
...
function render(){
 ...
 for(const [i,item]of batch.entries()){
  ...
  const comparison=document.createElement('div');comparison.className='comparison';comparison.append(imagePreview(item.file,'原图',item.source));row.append(comparison);
  if(item.status==='done'){
   ...
   const label=document.createElement('label'),check=document.createElement('input');check.type='checkbox';check.checked=item.reviewed;check.setAttribute('aria-label',`确认图片 ${item.source}`);
   check.onchange=()=>{item.reviewed=check.checked;updateControls();void save();};label.append(check,document.createTextNode('已检查边缘，可以交付'));row.append(document.createElement('br'),label);
  }else{
...
function infer(bitmap){
 return new Promise((resolve,reject)=>{
  try{worker??=new Worker(new URL('./worker.mjs',import.meta.url),{type:'module'});}catch(error){bitmap.close();reject(error);return;}
  const id=++request;
 ...
 try{
  for(const item of batch){
   ...
   try{
    ...
    item.output=await infer(bitmap);item.status='done';
...
exportBatch.onclick=async()=>{
 ...
 try{
  ...
  if(total>128*1024*1024)throw Error('此批交付超过 128 MB，请分批或逐图下载');
...
try{
 ...
 if(snapshot){batch=snapshot.items;batchSize=snapshot.size;size.value=String(batchSize);reviewedOnly.checked=snapshot.reviewedOnly;saveStatus.textContent=`已恢复 ${batch.length} 张图片，未完成项可继续处理`;}
