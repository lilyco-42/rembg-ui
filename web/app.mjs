...
run.onclick=async()=>{
 ...
 try{for(const [i,file]of selected.entries()){
  ...
  try{
   ...
   const image=document.createElement('img');image.alt=`${file.name} 的抠图结果`;image.src=URL.createObjectURL(output.product);urls.push(image.src);row.append(image,document.createElement('br'));
  ...
  }catch(error){const p=document.createElement('p');p.textContent=`失败：${error.message}`;row.append(p);}
