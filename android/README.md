...
- 工具栏「授权与额度」可粘贴购买页签发的 `ol1` 令牌；应用内置公钥在本地验证 Ed25519 签名，成功推理按月扣减本机额度，失败会回滚。联网时只把授权令牌提交到托管验证接口，用于发现退款/撤销；网络失败保持离线可用。令牌、安装摘要和额度只保存在应用私有存储，不上传原图，也不包含支付密钥。
...
工具栏的「购买专业版」会打开积分购买页；原生端不在 APK 内保存支付密钥或管理员凭据。付款和授权签发仍由服务端完成，Android 与桌面端使用同一 `ol1` 公钥校验格式。Android 的月度账本是离线本机计量，托管服务负责跨设备汇总、撤销和退款；Android 会在联网恢复后刷新授权状态，但不会上传原图。
