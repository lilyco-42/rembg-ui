...
| Android arm64-v8a / armeabi-v7a / x86_64 | `android/gradlew` | 3 个 ABI 拆分 APK + AAB（AAB 另含依赖自带的 `x86`，Play 会为每个设备单独出包） | 原生端侧单图与按套餐放开的顺序批处理（试用 10 / 创作者 50 / 小团队 200，统一封顶 200）、原图/结果切换、系统分享接收、可停止与重启恢复、ZIP 清单导出、除内置 `u2netp` 外的模型需联网从上游下载、`ol1` 公钥离线授权、本机月度额度和联网撤销刷新 | 真机耗时、峰值内存、相册分享、后台切换和签名安装；本机额度不能替代跨设备托管计量；上游模型文件的长期可用性与是否需要镜像 |
