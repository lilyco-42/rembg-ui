...
| Android arm64-v8a / armeabi-v7a / x86_64 | `android/gradlew` | ABI 拆分 APK + AAB | 原生端侧单图与最多 10 张顺序批处理、原图/结果切换、系统分享接收、可停止与重启恢复、ZIP 清单导出、模型按需下载、`ol1` 公钥离线授权、本机月度额度和联网撤销刷新 | 真机耗时、峰值内存、相册分享、后台切换和签名安装；本机额度不能替代跨设备托管计量 |
