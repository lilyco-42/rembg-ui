...
lain42.top 的 `studio-billing` 已部署 USDT/TRC20 试验订单、卡密兑换、Rembg `ol1` 自动签发和独立积分账本入口，但这只是首批试单通道，尚未完成真实订单验收。积分接口支持管理员幂等加分、用户消费和流水查询，暂未接入推理配额。支付宝、微信、Stripe、正式商家主体、服务地域和退款规则仍需由经营者冻结；扩大经营前要为订单回调增加幂等、金额校验、退款撤销和审计记录。
