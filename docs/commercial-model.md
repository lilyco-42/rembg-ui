...
lain42.top 的 `studio-billing` 已部署 USDT/TRC20 试验订单、USDT 充值积分、Rembg 积分兑换、卡密兑换和 `ol1` 自动签发，但这只是首批试单通道，尚未完成真实订单验收。积分账本支持管理员幂等加分、USDT 链上入账、Rembg 原子扣分兑换和流水查询；桌面端现在用本机 SQLite 账本按月原子预占/回滚推理额度，单图、批处理、SAM 与服装分层共用同一额度。支付宝、微信、Stripe、正式商家主体、服务地域和退款规则仍需由经营者冻结；扩大经营前要为订单回调增加唯一订单绑定、金额校验、退款撤销和审计记录。
